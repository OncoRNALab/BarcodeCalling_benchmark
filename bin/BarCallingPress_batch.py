...
parser.add_argument ("--barcode-start", dest="Bar_start", type=int, default=0, help="Starting position of barcode in read (e.g., 10 if after 10nt UMI)")
...
# Count lines in input file (handle both gzipped and uncompressed)
if read1.endswith('.gz'):
    cat_cmd = ['zcat', read1]
else:
    cat_cmd = ['cat', read1]
out = subprocess.Popen(cat_cmd, stdout=subprocess.PIPE, stderr=subprocess.STDOUT)
...
totlines = int(val)
print(f"DEBUG: Total lines in {read1}: {totlines}, Total reads: {totlines//4}")
#parse segment ID
...
def bar_demux (seq):
  ...
  ibest_int = int(ibest.item())  # Convert torch tensor to Python int
  if ans[ia] <= Nthresh:
    # Return tuple: (barcode_index, barcode_sequence)
    return (ibest_int, codes[ibest_int])
  else:
    return ("erasure", None)

print("Starting Demux")
print(f"DEBUG: Processing reads from {startline} to {endline} (total expected: {endline - startline})")

# Start timing for barcode decoding
...
with open_maybe_gz(read1) as r1_handle, open_maybe_gz(read2) as r2_handle, \
    ...
    for r1_lines, r2_lines in zip(fastq_iter(r1_handle), fastq_iter(r2_handle)):
        ...
        if reads_processed <= 3:
            print(f"DEBUG Read {reads_processed}: R1_header={r1_lines[0].strip()}, R1_seq_len={len(read)}, first_28bp={read[:28]}")

        # 3) grab exactly M bases starting from barcode_start position (pad on the right with 't' if too short)
        if len(read) < barcode_start + M:
            # Read too short to extract full barcode from correct position
            raw = read[barcode_start:]
            raw = raw.ljust(M, "t")
        else:
            # Extract M bases starting at barcode_start
            raw = read[barcode_start:barcode_start+M]

        # 4) encode & demux
        seq = encode(raw)
        demux_result = bar_demux(seq)

        # 5) if we got a barcode, trim, rename, write
        if demux_result[0] != "erasure":
            called_idx, barcode_seq = demux_result
            reads_called += 1
            if reads_called <= 3:
                print(f"DEBUG: Read {reads_processed} CALLED as barcode index {called_idx}: {barcode_seq.strip()}")
            # trim the barcode (barcode_start + M bases) off R1
            trimmed_r1_seq = r1_lines[1][barcode_start+M:]
            trimmed_r1_qual = r1_lines[3][barcode_start+M:]
            
            # Handle both simulated and real FASTQ header formats
            header = r2_lines[0].strip()
            if "_" in header and header.startswith("@read_"):
                # Simulated data format: @read_0_barcode_13828
                prefix, rest = header.split("_", 1)
                newname = f"{prefix}_calledidx_{called_idx}_{barcode_seq.strip().upper()}_{rest}\n"
            else:
                # Real sequencing data format: @M04784:814:000000000-GR2FH:1:1102:15533:1332
                # Append barcode info to original header
                newname = f"{header}_calledidx_{called_idx}_{barcode_seq.strip().upper()}\n"
            # Write to output
            ...
            reads_erasure += 1
            if reads_erasure <= 3:
                print(f"DEBUG: Read {reads_processed} marked as ERASURE")
        i += 1
...
# Use builtin max to avoid numpy.max axis interpretation issue
denom = builtins.max(1, int(reads_processed))
reads_called_pct = 100.0 * int(reads_called) / denom
reads_erasure_pct = 100.0 * int(reads_erasure) / denom
print(f"Reads with barcode call: {reads_called} ({reads_called_pct:.2f}%)")
print(f"Reads marked as erasure: {reads_erasure} ({reads_erasure_pct:.2f}%)")
print(f"Barcode decoding completed in {decode_duration:.3f} seconds")
if reads_processed > 0:
    time_per_read_ms = (decode_duration / reads_processed) * 1000
    print(f"Time per read: {time_per_read_ms:.6f} ms")
